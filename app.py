...
def proses1():
    ...
    if 'otsu' in request.form.get('select_thresholding'):
        ...
        mode = 'blur'
    elif 'grayscale' in request.form.get('select_thresholding'):
        mode = 'grayscale'
    elif 'lapciangradient' in request.form.get('select_thresholding'):
        mode = 'lapciangradient'
    ...
    if mode == 'otsu':
        ...
        img_res = process.img_gray(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
    elif mode == 'lapciangradient':
        img_res = process.lapcian_gradient(img)
        cv2.imwrite("/".join([target, 'result.jpg']),img_res)
